...
class LangChainAnalysisProvider:
    ...
    def _build_surgical_rewrite_prompt(
        ...
        return (
            ...
            "When a suggestion includes a non-empty `reviewer_notes` list, those notes are direct instructions "
            "from the human reviewer and must be followed. If a reviewer note conflicts with the agent's "
            "`suggestion`, the reviewer note wins.\n"
            "When the `author_label` identifies a human reviewer (for example `Workspace reviewer`) and "
            "`suggestion` is empty, treat the `comment` field itself as a hard reviewer instruction scoped to "
            "the `quote`.\n\n"
