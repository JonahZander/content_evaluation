...
from content_evaluation.domain.models import ArtifactBlock, ArtifactDocument, RunInput


def normalize_text(input_data: RunInput, extracted_text: str, title: str | None = None) -> ArtifactDocument:
    ...
    blocks = [ArtifactBlock(index=index, text=text) for index, text in enumerate(paragraphs)]
